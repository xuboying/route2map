...
def IPToNum(ip):
    r = struct.unpack(">I", struct.pack("BBBB", * map( lambda a : int(a), ip.split("."))))
    return r[0]
def GetAddress(ip):
    global index
    ...
    with open(index, 'rb') as idx:
        while True:
            M = int((int((U / SEGLENGTH - L / SEGLENGTH) / 2) + L / SEGLENGTH) * SEGLENGTH)
            idx.seek(M)
            x = idx.read(4)
            n = struct.unpack(">I", x)
            n = n[0]
    ...
    while True:
        ...
        if M:
            ...
            if Addr != "Local" and M.group(2):
                ...
                print("%s%-15s%s %s %s" % (M.group(1), M.group(3), Addr, addr1, coordinate))
            else:
                print("%s%-15s%s" % (M.group(1), M.group(3), Addr))
        else:
            print(line, end = "")
    coordinates = unique(coordinates)
    coordinates = list(map( lambda x : list(map( lambda y : float(y), x.split(","))), coordinates))
